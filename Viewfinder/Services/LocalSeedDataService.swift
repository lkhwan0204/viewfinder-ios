...
enum HomeRecommendationKind: String, CaseIterable, Identifiable, Sendable {
    case sunset
    case night
    case cafe
    case seasonal
...
struct HomeRecommendationSnapshot {
...
struct HomeRecommendationService {
    private let seedService: LocalSeedDataService
    ...
    func makeSnapshot(
        ...
        let baseCandidates = seedService.allPhotoSpots()
    ...
    private func logEmptyHomeSection(kind: HomeRecommendationKind, strictCandidateCount: Int) {
        ...
        print("[Viewfinder][HomeRecommendations] \(kind.title) 후보 0개: \(reason) strictCandidates=\(strictCandidateCount)")
    }
    ...
    private func logRecommendationSnapshot(
        ...
        print("[Viewfinder][HomeRecommendations] coordinate=\(coordinateText) radius=\(radiusText) candidateCount=\(candidateCount) final=[\(names)] fallbackUsed=\(fallbackUsed)")
    }
}
