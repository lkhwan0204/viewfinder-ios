...
enum AuthSignInError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .secureRandomFailed:
            return "보안 로그인 정보를 만들지 못했어요. 잠시 후 다시 시도해주세요."
...
final class FirebaseAuthService: AuthService {
                ...
                if let error {
                    if Self.isGoogleSignInCancelled(error) {
                        ...
                        AppLog.authentication.error(
                            "Google sign-in failed: \(error.localizedDescription, privacy: .public)"
                        )
    ...
    private func upsertUserDocument(uid: String, provider: AuthProviderKind, displayName: String, email: String?) async {
        ...
        do {
            ...
            AppLog.persistence.error(
                "Firestore user upsert failed: \(error.localizedDescription, privacy: .public)"
            )
        }
    }
    ...
    private func updateFirebaseProfileIfNeeded(user: FirebaseAuth.User, displayName: String) async {
        ...
        do {
            ...
            AppLog.persistence.error(
                "Firebase profile update failed: \(error.localizedDescription, privacy: .public)"
            )
        }
    }
...
final class AppleSignInCoordinator: NSObject, ASAuthorizationControllerDelegate, ASAuthorizationControllerPresentationContextProviding {
    ...
    func start() {
        ...
        do {
            ...
            AppLog.persistence.error("Secure nonce generation failed")
            completion(.failure(.secureRandomFailed))
            return
        }
        rawNonce = nonce
    ...
    func authorizationController(controller: ASAuthorizationController, didCompleteWithAuthorization authorization: ASAuthorization) {
        guard let credential = authorization.credential as? ASAuthorizationAppleIDCredential,
              let rawNonce else {
            AppLog.authentication.error("Apple sign-in failed: missing credential or nonce")
    ...
    func authorizationController(controller: ASAuthorizationController, didCompleteWithError error: Error) {
        ...
        if nsError.domain == ASAuthorizationError.errorDomain,
           ...
           code == .canceled {
            AppLog.authentication.info(
                "Apple sign-in cancelled: \(message, privacy: .public)"
            )
        ...
        if nsError.domain == ASAuthorizationError.errorDomain,
           ...
           code == .unknown {
            AppLog.authentication.error(
                "Apple sign-in capability failure: \(message, privacy: .public)"
            )
        ...
        AppLog.authentication.error(
            "Apple sign-in failed: \(message, privacy: .public)"
        )
    ...
    private static func randomNonceString(length: Int = 32) throws -> String {
        ...
        while remainingLength > 0 {
            ...
            if status != errSecSuccess {
                throw AuthSignInError.secureRandomFailed
            }
...
final class AuthViewModel: ObservableObject {
    ...
    func prepareForSignInPresentation() {
        errorMessage = nil
    }
    ...
    private func handle(_ error: Error, provider: AuthProviderKind) {
        ...
        if provider == .apple,
           ...
           code == .canceled {
            AppLog.authentication.info(
                "Apple sign-in cancelled: \(error.localizedDescription, privacy: .public)"
            )
        ...
        if provider == .apple,
           ...
           code == .unknown {
            AppLog.authentication.error(
                "Apple sign-in capability failure: \(error.localizedDescription, privacy: .public)"
            )
        ...
        AppLog.authentication.error(
            "\(provider.title, privacy: .public) sign-in failed: \(error.localizedDescription, privacy: .public)"
        )
